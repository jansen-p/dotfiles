...
class Tracker:
        ...
        if init:
            ...
            try:
                ...
                if not os.path.isfile(self.src_path + ".navi_hist/" + loc_name):
                    print(
                        "Didn't find history for this dir!\nInitialize one with 'init'."
                    )
                    exit()

    # returns a list of shortened subdirectories, given a list of subdirectories (full paths)
    def get_shortened_dirs(
        self, dirs
    ):  # example: /run/Parts/Exch/Sync/uni/Studium/6.Sem/Grundlagen_ML/exercises -> Grundlagen_ML/exercises
        paths_short = []
        for f in dirs:
    ...
    def remove_dict(self):
        try:
            os.remove(self.src_path + ".navi_hist/" + self.loc_name)
            print("Removed dictionary for", self.loc_name)
        except:
            print("Failed to remove dictionary")
    ...
    def init(self):
        ...
        for i in dic:
            print(
                "  - " + i
            )  # need " " otherwise bash won't find whitespace (and won't print it)
        return dic
    ...
    def set_dict_score(self, num, score):
        ...
        if num == ".":
            ...
            if len(k) == 0:
                ...
                return
            print(
                "entry " + bcolors.OKGREEN + str(k) + bcolors.ENDC + ": assigned score",
                score,
            )
            self.data[k] = score
        else:
    ...
    def find_hit(self, inp, match: Callable, match_file=False, response=False):
        ...
        for row in sorted_tuples:
            ...
            key = row[0].lower()[start:]

            if match_file:
                folder = self.current_location + "/" + row[0]
                files = [
                    f
    ...
    def rescale(self, minimum=0, maximum=20):
        vals = (
            lambda x: [
                int(((k - min(x)) * maximum) / (max(x) - min(x)) + minimum) for k in x
            ]
        )(self.data.values())
    ...
    def print(self, num):
        ...
        t.align["Path"] = "l"
        for x, index in zip(
            self.sort_dict()[:num], range(num if num != None else len(self.data))
        ):
